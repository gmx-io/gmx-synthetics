 ...
 *
 * Run:
 * - `DEPLOYMENT_NETWORK=arbitrum npx hardhat run --no-compile scripts/scanAddressParamExternalCalls.ts`
 *
 * Optional filters:
...
type DeploymentJson = { abi?: unknown; deployedBytecode?: string };
...
async function main() {
  ...
  for (const file of files) {
    ...
    const flaggedFunctions: Array<{ signature: string; usedParamNames: string[]; inconclusive: boolean }> = [];
    ...
    if (flaggedFunctions.length > 0) {
      totalFlaggedContracts++;
      totalFlagged += flaggedFunctions.length;
    }
    ...
    for (const match of flaggedFunctions.sort((a, b) => a.signature.localeCompare(b.signature))) {
      ...
      const note = match.inconclusive ? " [inconclusive scan]" : "";
      console.log(`  - ${match.signature}${suffix}${note}`);
    }
  ...
  console.log(
    `Done. Flagged ${totalFlagged} function(s) across ${totalFlaggedContracts} contract(s). Inconclusive scans: ${totalInconclusive}${
      totalInconclusive > 0 && !printInconclusive ? " (set PRINT_INCONCLUSIVE=1 to list)" : ""
    }.`
  );
}
