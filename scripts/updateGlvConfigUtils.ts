...
import { DEFAULT_MARKET_TYPE, getMarketTokenAddress, getOnchainMarkets } from "../utils/market";
...
const processGlvs = async ({ markets, glvs, onchainMarketsByTokens, tokens, generalConfig, handleConfig }) => {
  ...
  for (const glvConfig of glvs) {
    ...
    const glvName = glvConfig.name ?? `GMX Liquidity Vault [${glvConfig.longToken}-${glvConfig.shortToken}]`;
    const glvSymbol = glvConfig.symbol ?? `GLV [${glvConfig.longToken}-${glvConfig.shortToken}]`;
    const glvType = glvConfig.type ?? hre.ethers.constants.HashZero;

    // should not use this because it doesn't work for markets
    ...
    for (const glvMarketConfig of glvConfig.markets) {
      ...
      const glvMarketType = DEFAULT_MARKET_TYPE;
      console.log(
        indexToken.address,
        longToken.address,
        shortToken.address,
      ...
      // TODO address is incorrect for some reason
      // probably because of different bytecode for MarketToken due to newer compiler?
      const marketAddress = getMarketTokenAddress(
        indexToken.address,
        longToken.address,
        shortToken.address,
        glvMarketType,
  ...
  //   await handleConfig(
  //     "uint",
  //     keys.glvShiftMinIntervalKey(glv),
