...
async function main() {
  ...
  writeSafeBatchJson(timelockMethod, safeBatchTransactions);
  await timelockWriteMulticall({ timelock, multicallWriteParams });
}

function writeSafeBatchJson(timelockMethod: string, transactions: any[]) {
  ...
  const filename = path.join(outDir, `safe-batch-${timelockMethod}-${hre.network.name}-${stamp}.json`);
  ...
  const batch = {
    ...
    createdAt: Date.now(),
    meta: {
      name: `${timelockMethod} (${transactions.length} txs)`,
      description: `Generated by scripts/withdrawFromPositionImpactPool.ts on ${hre.network.name}`,
  ...
  console.log(`Safe Transaction Builder batch written to: ${filename}`);
