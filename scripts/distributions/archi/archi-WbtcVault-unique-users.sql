-- WBTC Vault User Summary Analysis
-- Contract: 0xee54A31e9759B0F7FDbF48221b72CD9F3aEA00AB (WBTC Vault)
-- Aggregates all activity per unique user
-- https://dune.com/queries/5818468

WITH wbtc_vault_transactions AS (
    -- Direct function calls to WBTC Vault
    SELECT
        t.block_number,
        t.block_time,
        t.hash as tx_hash,
        ...
        -- Note: WBTC has 8 decimals, not 18 like WETH
        CASE
            WHEN LENGTH(t.data) >= 36 AND bytearray_substring(t.data, 1, 4) IN (
                ...
                0x2e1a7d4d, -- withdraw
                0xb6b55f25  -- deposit
            ) THEN bytearray_to_uint256(bytearray_substring(t.data, 5, 32)) / 1e8  -- WBTC has 8 decimals
            ELSE NULL
...
SELECT
    ...
    -- Assets summary
    ROUND(total_deposited, 8) as total_deposited_wbtc,  -- 8 decimals for WBTC precision
    ROUND(total_withdrawn, 8) as total_withdrawn_wbtc,
    ROUND(net_position, 8) as net_position_wbtc,
