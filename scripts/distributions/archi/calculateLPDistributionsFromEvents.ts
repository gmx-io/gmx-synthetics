...
// NOTE: This script is now fully self-contained and requires NO external CSV files!

// npx hardhat run --network arbitrum scripts/distributions/archi/calculateLPDistributionsFromEvents.ts

/**
 * LP DISTRIBUTIONS (EVENT-BASED): Complete self-contained calculation
 *
 * This is the CORRECT approach - fully self-contained, no CSV dependencies:
 * 1. Query farmer positions from CreditUser contract to calculate vault borrowing
 * 2. Query AddLiquidity/RemoveLiquidity EVENTS from Vault contracts (not transactions!)
 * 3. Extract _recipient addresses from events (the actual LP, not msg.sender)
 ...
 *
 * Why this is better than the Dune CSV:
 * - Events capture the _recipient parameter, which is the actual LP
 * - Transactions only show msg.sender, which might be a contract
 * - On-chain balances are current state of truth
 * - No external dependencies - all data from blockchain
 *
 * Outputs:
 *   - lp-distributions-events.csv: LP distributions from events
 *   - lp-distributions-events-by-vault.csv: Detailed breakdown
 */
...
const VAULT_CONFIGS = [
  {
    ...
    supplyRewardPool: "0x9eBC025393d86f211A720b95650dff133b270684",
    decimals: 18,
  ...
  },
  {
    ...
    supplyRewardPool: "0x12e14fDc843Fb9c64B84Dfa6fB03350D6810d8e5",
    decimals: 8,
  ...
  },
  {
    ...
    supplyRewardPool: "0xEca975BeEc3bC90C424FF101605ECBCef22b66eA",
    decimals: 6,
    deployBlock: 76000000,
  },
  {
    ...
    supplyRewardPool: "0x670c4391f6421e4cE64D108F810C56479ADFE4B3",
    decimals: 6,
    deployBlock: 76000000,
  },
];
...
async function main() {
  console.log("\n" + "=".repeat(80));
  console.log("LP DISTRIBUTIONS (EVENT-BASED): Complete Self-Contained Calculation");
  ...
  // ========================================================================
  // STEP 2: Query Vault events to find all LP addresses
  // ========================================================================

  console.log("Step 2: Querying Vault events to discover LP addresses...\n");
  ...
  for (const config of VAULT_CONFIGS) {
    console.log("=".repeat(80));
    console.log(`${config.name} Vault (${config.vaultAddress})`);
    console.log("=".repeat(80) + "\n");

    const vault = new ethers.Contract(config.vaultAddress, VAULT_ABI, provider);

    console.log(`  Querying AddLiquidity events from block ${config.deployBlock}...`);
    const addLiquidityFilter = vault.filters.AddLiquidity();
    const addEvents = await vault.queryFilter(addLiquidityFilter, config.deployBlock, currentBlock);
    console.log(`  Found ${addEvents.length} AddLiquidity events`);

    console.log(`  Querying RemoveLiquidity events from block ${config.deployBlock}...`);
    const removeLiquidityFilter = vault.filters.RemoveLiquidity();
    const removeEvents = await vault.queryFilter(removeLiquidityFilter, config.deployBlock, currentBlock);
    console.log(`  Found ${removeEvents.length} RemoveLiquidity events`);
  ...
  const vaultResults = await Promise.all(
    vaultOrder.map(async (vaultName) => {
      ...
      const baseRewardPool = new ethers.Contract(config.supplyRewardPool, BASE_REWARD_ABI, provider);
  ...
  const detailedPath = path.join(__dirname, "lp-distributions-events-by-vault.csv");
  const detailedRows = [
  ...
  console.log(`✅ lp-distributions-events-by-vault.csv (${nonZeroDistributions.length} LPs)`);

  // Output 2: Aggregated
  const aggregatedPath = path.join(__dirname, "lp-distributions-events.csv");
  ...
  console.log(`✅ lp-distributions-events.csv (${nonZeroDistributions.length} LPs)\n`);
  ...
  const accuracy = grandTotal.mul(10000).div(totalExpected).toNumber() / 100;
  console.log(`Distribution accuracy: ${accuracy.toFixed(10)}%\n`);
...
**Note:** Full LP distribution list available in \`lp-distributions-events.csv\` (${lpDistributions.length} LPs total)`;
