-- USDC Vault User Summary Analysis
-- Contract: 0xa7490e0828Ed39DF886b9032ebBF98851193D79c (USDCVaultProxy)
-- Aggregates all activity per unique user
-- https://dune.com/queries/5818483

WITH usdc_vault_transactions AS (
    -- Direct function calls to USDC Vault
    SELECT
        t.block_number,
        t.block_time,
        t.hash as tx_hash,
        ...
        CASE
            WHEN LENGTH(t.data) >= 36 AND bytearray_substring(t.data, 1, 4) IN (
                ...
                0x2e1a7d4d, -- withdraw
                0xb6b55f25  -- deposit
            ) THEN bytearray_to_uint256(bytearray_substring(t.data, 5, 32)) / 1e6  -- USDC has 6 decimals
            ELSE NULL
...
SELECT
    ...
    -- Assets summary
    ROUND(total_deposited, 2) as total_deposited_usdc,  -- 2 decimals for USD display
    ROUND(total_withdrawn, 2) as total_withdrawn_usdc,
    ROUND(net_position, 2) as net_position_usdc,
