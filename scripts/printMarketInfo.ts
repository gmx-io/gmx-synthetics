...
async function main() {
  ...
  for (const market of markets) {
    for (const [prop, key] of [
      ...
      ["minFundingIncreaseRatePerSecond", keys.minFundingIncreaseRatePerSecondKey(market.marketToken)],
      ...
      ["minFundingFactorPerSecondLong", keys.minFundingFactorPerSecondKey(market.marketToken, true)],
  ...
  for (let i = 0; i < marketInfoList.length; i++) {
    ...
    if (indexToken) {
      data = {
        ...
        "min fund incr rate h": formatAmount(marketValues.minFundingIncreaseRatePerSecond.mul(3600), 30, 10),
        "fund decr rate h": formatAmount(marketValues.fundingDecreaseFactorPerSecond.mul(3600), 30, 10),
        "min fund rate h long": formatAmount(marketValues.minFundingFactorPerSecondLong.mul(3600), 30, 10),
        "min fund rate h short": formatAmount(marketValues.minFundingFactorPerSecondShort.mul(3600), 30, 10),
        "max fund rate h long": formatAmount(marketValues.maxFundingFactorPerSecondLong.mul(3600), 30, 10),
        "max fund rate h short": formatAmount(marketValues.maxFundingFactorPerSecondShort.mul(3600), 30, 10),
