...
describe("Exchange.BorrowingFees", () => {
  ...
  it("borrowing fees", async () => {
    ...
    await usingResult(
      getMarketTokenPriceWithPoolValue(fixture, { prices: prices.ethUsdMarket }),
      ([marketTokenPrice, poolValueInfo]) => {
        expect(marketTokenPrice).eq("1001068487605242432177935697848"); // 1.00106848761
  ...
  it("single token market", async () => {
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        expect(marketTokenPrice).eq("1000483841519800000000000000000"); // 1.00048384152
        expect(poolValueInfo.poolValue).eq("10004838415198000000000000000000000000"); // 10,004,838.4152
      }
    );
  });
});
