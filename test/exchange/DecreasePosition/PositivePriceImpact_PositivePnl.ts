...
describe("Exchange.DecreasePosition", () => {
  ...
  it("positive price impact, positive pnl", async () => {
    ...
    // the impact pool increased by 0.000430279083665338 ETH, 2.15 USD
    expect(await dataStore.getUint(keys.positionImpactPoolAmountKey(ethUsdMarket.marketToken))).eq("430279083665338"); // 0.000430279083665338 ETH
    expect(await dataStore.getInt(getPendingImpactAmountKey(positionKey0))).eq("-71999999999999999"); // -0.072
    ...
    expect(await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)).eq("1000002160001"); // 1_000_002.16
    ...
    await usingResult(
      ...
      (positionInfo) => {
        expect(positionInfo.position.numbers.collateralAmount).eq("49997839999"); // 49,997.84
    ...
    await usingResult(
      getMarketTokenPriceWithPoolValue(fixture, { prices: prices.ethUsdMarket }),
      ([marketTokenPrice, poolValueInfo]) => {
        expect(marketTokenPrice).eq("999986721221780212484166666666");
        expect(poolValueInfo.poolValue).eq("5999920327330681274905000000000000000");
    ...
    await usingResult(
      getMarketTokenPriceWithPoolValue(fixture, { prices: prices.ethUsdMarket.increased }),
      ([marketTokenPrice, poolValueInfo]) => {
        expect(marketTokenPrice).eq("1003200000000000000000770000000");
        expect(poolValueInfo.poolValue).eq("6019200000000000000004620000000000000");
