...
library Keys {
    ...
    bytes32 public constant DATA_STREAM_SPREAD_FACTOR = keccak256(abi.encode("DATA_STREAM_SPREAD_FACTOR"));
   ...
   function virtualInventoryForSwapsKey(bytes32 virtualMarketId, bool isLongToken) internal pure returns (bytes32) {
    ...
    function dataStreamSpreadFactorKey(address token) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            DATA_STREAM_SPREAD_FACTOR,
            token
        ));
    }
