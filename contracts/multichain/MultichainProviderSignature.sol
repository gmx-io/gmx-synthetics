...
contract MultichainProviderSignature is EIP712 {
    ...
    // Message(address token,uint256 amount,address account,uint256 srcChainId,uint32 srcEid)
    bytes32 private constant _MESSAGE_TYPEHASH =
        keccak256("Message(address token,uint256 amount,address account,uint256 srcChainId,uint32 srcEid)");

    constructor() EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {}

    /**
     * Check the signature for a given message
     * @param message The ABI encoded parameters (token, amount, account, srcChainId, srcEid).
     * @param signature The EIP-712 signature.
     */
    function isSigner(bytes calldata message, bytes calldata signature) external view returns (bool) {
        // Decode the message
        (address token, uint256 amount, address account, uint256 srcChainId, uint32 srcEid) = MultichainProviderUtils
